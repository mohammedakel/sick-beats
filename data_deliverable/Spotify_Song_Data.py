...
def getTrackId(song_title, artisit_name, max_result=5):
	...
	params = 'artist:' + artisit_name + ' track:' + song_title
...
def getSongAudioFeatures(Features_list): 
	...
	track_dancability = 0.0 
	track_liveness = 0.0 
	track_Loudness = 0.0 
	track_speechniess = 0.0
	...
	for song in Features_list:
			track_dancability = song['danceability']
			track_liveness = song['liveness']
			track_Loudness = song['loudness']
			track_speechniess = song['speechiness']
...
def getSongData(song_title, artisit_name, max_result):
	...
	tracks_list = getTrackId(song_title, artisit_name, max_result)
	processed_tracks = []
	for track in tracks_list:	
		...
		track_dancability = audio_features_map["dancibility"]
		track_liveness = audio_features_map["liveness"]
		track_Loudness = audio_features_map["loudness"]
		track_speechniess = audio_features_map["speechiness"]
